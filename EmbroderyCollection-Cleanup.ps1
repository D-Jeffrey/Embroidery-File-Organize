...
if (!( test-path -Path $FilesCollection )) {
    Write-Host "Can not find the Main directory for $$CollectionTypeofStr ($FilesCollection).  Stopping" -BackgroundColor DarkRed -ForegroundColor White
    $failed = $true
    }
    
if (!( test-path -Path $EmbrodRootDir)) {
    Write-Host "Can not find the main directory ($EmbrodRootDir) within $FilesCollection.  Stopping" -BackgroundColor DarkRed -ForegroundColor White
